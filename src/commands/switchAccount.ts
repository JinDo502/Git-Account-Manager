...
import { getCurrentRepoInfo, setGitConfig, updateRemoteUrl, createInitialCommit, pushToRemote, checkRepoExists, deleteGitHubRepo } from '../utils/git';
import { selectAccount, confirm, input, confirmDangerousAction } from '../utils/interactive';
 ...
 * @param options 选项
 */
export async function switchAccount(accountName?: string, options: SwitchOptions = {}): Promise<void> {
  try {
    // 如果指定了--migrate选项，执行迁移流程
    if (options.migrate) {
      await migrateRepo(accountName, options);
      return;
    }
    ...
    const customRepoName = options.name || (await input('输入仓库名称 (留空使用当前目录名):', repoInfo.name));
    ...
    if (shouldCreateRemote) {
      ...
      isPrivate = options.private !== undefined ? options.private : !(await confirm('是否将仓库设置为公开(Public)仓库? 选择"否"则创建为私有(Private)仓库', false));
 ...
 * @param options 选项
 */
async function migrateRepo(targetAccountName?: string, options: SwitchOptions = {}): Promise<void> {
  try {
    // 获取源账号（当前账号）
    const { name: sourceAccountName } = getAccount();
    ...
    if (options.deleteSource) {
      const { account } = getAccount(sourceAccountName);
      sourceAccount = account;

      // 如果要删除源仓库，检查源账号是否有GitHub令牌
      if (!sourceAccount.githubToken) {
    ...
    if (options.deleteSource && !shouldInit && sourceAccount) {
